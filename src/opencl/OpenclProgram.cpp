...
OpenclProgram::OpenclProgram(std::unique_ptr<cl_program>& p_handle)
...
std::unique_ptr<OpenclKernel> OpenclProgram::createKernel(const std::string& p_name) const throw (OpenclError) {
	cl_int error;
	std::unique_ptr<cl_kernel> kernel(new cl_kernel);
	*kernel = clCreateKernel(*m_handle, p_name.c_str(), &error);
	if(error != CL_SUCCESS) {
